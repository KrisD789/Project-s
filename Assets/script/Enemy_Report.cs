...
public class Enemy_Report : MonoBehaviour
{
    ...
    void Update()
    {
        if (enemy_script.currentState == enemy.EnemyState.report)
        {
            FindClosestFriend();

            // ทดสอบ: ลากเส้นสีเขียวไปหาเพื่อนที่ใกล้ที่สุด
            if (closestFriend != null && enemy_script.currentState == enemy.EnemyState.report)
            {
    ...
    void FindClosestFriend()
    {
        ...
        foreach (var col in friendsInArea)
        {
            ...
            enemy friendScript = col.GetComponent<enemy>();

            if (friendScript != null && friendScript.wasFaint != true)
            {
                if (friendScript.currentState != enemy.EnemyState.faint || friendScript.currentState != enemy.EnemyState.dead)
                {
